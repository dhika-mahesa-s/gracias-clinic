@extends('layouts.app') {{-- Ganti ke layouts.dashboard jika layout admin berbeda --}}
@extends('layouts.app')
...
<div class="px-4 py-8">
    ...
    <div class="bg-white p-6 rounded-xl shadow-lg mb-8 border border-gray-100">
        <div class="flex items-center space-x-3 mb-4">
            ...
            <h1 class="text-2xl md:text-3xl font-semibold text-gray-800">Manajemen Reservasi</h1>
        </div>
<div class="px-4 py-8">
    ...
    <div class="bg-white p-6 rounded-xl shadow-lg mb-8 border border-gray-100">
        ...
        <form action="{{ route('reservations.history') }}" method="GET" class="flex items-center bg-gray-50 rounded-lg p-3 border border-gray-200">
            ...
            <input type="text" name="search" class="flex-grow focus:outline-none text-gray-700 bg-transparent text-sm" placeholder="Cari Pasien, Kode Booking, Treatment, atau Dokter..." value="{{ request('search') }}">
        ...
        <form id="filter-form-admin" action="{{ route('admin.reservations.history') }}" method="GET">
            ...
            <div class="flex flex-wrap items-center space-x-2 mt-4 text-sm">
                <label for="status-filter-admin" class="text-gray-600 font-medium">Filter:</label>
                <select name="status" id="status-filter-admin" onchange="document.getElementById('filter-form-admin').submit()" class="px-3 py-1.5 border border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
        ...
        {{-- Tombol Cetak Laporan (DITAMBAHKAN) --}}
