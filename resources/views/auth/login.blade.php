    ...
    <div class="min-h-screen flex items-center justify-center bg-background text-foreground py-12 px-6">
        <div class="w-full max-w-md bg-card rounded-2xl shadow-xl p-8 border border-border">
            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-5">
                ...
                {{-- 🌟 Tombol Login Google --}}
                <a href="{{ route('auth.redirect') }}" id="google-login-btn"
...
<script>
    document.addEventListener("DOMContentLoaded", () => {
        ...
        // Tambah input hidden untuk login manual
        ...
        // Update href login Google
        const googleBtn = document.getElementById("google-login-btn");
        if (googleBtn && intended) {
            googleBtn.href = googleBtn.href + "?redirect_to=" + encodeURIComponent(intended);
