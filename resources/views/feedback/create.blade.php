...
@section('content')
    ...
    <div class="py-16 bg-gradient-to-br from-blue-50 via-gray-100 to-gray-200">
        <div class="container mx-auto px-6 lg:px-8">
            ...
            <!-- Form -->
            <div
                class="bg-white/80 backdrop-blur-lg border border-gray-200 shadow-2xl rounded-2xl max-w-5xl mx-auto p-8 md:p-12">
                <form action="{{ route('feedback.store') }}" method="POST" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="md:col-span-2">
                            <label for="reservation_id" class="block font-medium text-center text-gray-700 mb-1">
                                Pilih Reservasi yang Ingin Diberi Feedback
                            </label>
                            <select name="reservation_id" id="reservation_id"
                                class="w-full px-4 py-2 rounded-lg border border-gray-300 text-center focus:ring-primary focus:border-primary transition duration-200">
                                ...
                                @else
                                    @foreach ($reservations as $r)
                                        <option value="{{ $r->id }}">
                                            {{ $r->treatment->name }} dengan {{ $r->doctor->name }} pada
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            ...
                            @foreach ($ratings as $field => $label)
                                <div>
                                    ...
                                    <div class="flex items-center gap-2 rating-stars text-xl text-gray-400"
                                        data-field="{{ $field }}">
                                        @for ($i = 1; $i <= 5; $i++)
