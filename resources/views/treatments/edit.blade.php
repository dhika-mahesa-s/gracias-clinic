        ...
        <form action="{{ route('treatments.update', $treatment) }}" method="POST" enctype="multipart/form-data"
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $treatment->name) }}"
                       class="w-full px-4 py-2 border border-gray-300 rounded-xl bg-white text-[#526D82] 
            ...
            <div>
                ...
                <input type="file" name="image" accept="image/*"
                       class="w-full px-4 py-2 border border-gray-300 rounded-xl bg-white text-sm text-[#526D82] 
                              focus:ring-2 focus:ring-primary focus:outline-none 
                              file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 
                              file:bg-primary file:text-white hover:file:bg-primary/90 transition-all duration-300">
                <small class="text-gray-500 block mt-2">Biarkan kosong jika tidak ingin mengganti gambar.</small>
            </div>

            <!-- Tombol -->
            <div class="flex justify-center gap-4 pt-4">
                <button type="submit"
                        class="flex items-center gap-2 px-6 py-2.5 bg-primary hover:bg-primary/90 
                               text-white rounded-xl font-medium shadow-md transition-all duration-300">
                    <i class="fa-solid fa-floppy-disk"></i> Simpan Perubahan
                </button>
                <a href="{{ route('treatments.manage') }}"
                   class="flex items-center gap-2 px-6 py-2.5 border border-[#526D82] text-[#526D82] 
                          rounded-xl hover:bg-[#526D82] hover:text-white shadow-sm transition-all duration-300">
